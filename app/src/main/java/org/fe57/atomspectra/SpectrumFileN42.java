...
public class SpectrumFileN42 extends SpectrumFile {
    ...
    public void loadSpectrum(@NonNull Uri histFile, Context context) {
        throw new UnsupportedOperationException("Load N42 is not supported");
    }
    ...
    public void saveSpectrumAndCloseStream(@NonNull OutputStreamWriter docStream, Context context) throws IOException, PackageManager.NameNotFoundException {
        ...
        if (backgroundSpectrum != null && !backgroundSpectrum.isEmpty()) {
            long[] tmp = backgroundSpectrum.getDataArray();
            ...
            if (!exportBackgroundCalibration.isCorrect()) {
                throw new InvalidParameterException("Background calibration is incorrect");
            }
