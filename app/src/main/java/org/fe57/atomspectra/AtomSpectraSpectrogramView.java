...
public class AtomSpectraSpectrogramView extends View {
    ...
    private static final int[] LIME_PALETTE = new int[]{
            0xFF020017, 0xFF0602AC, 0xFF0602AC, 0xFF0605AC, 0xFF0708AD, 0xFF0708AD, 0xFF070BAD, 0xFF070EAD, 0xFF0810AE, 0xFF0810AE, 0xFF0812AE, 0xFF0914AE, 0xFF0914AE, 0xFF0916AF, 0xFF0A18AF, 0xFF0A1AAF,
    ...
    private static final int[] YELLOW_PALETTE = new int[]{
            0xFF150200, 0xFF350300, 0xFF350400, 0xFF340400, 0xFF340500, 0xFF340600, 0xFF340600, 0xFF330700, 0xFF330800, 0xFF330900, 0xFF330900, 0xFF330A00, 0xFF330A00, 0xFF330B00, 0xFF330B00, 0xFF330C00,
    ...
    private static final int[] GLOW_PALETTE = new int[]{
            0xFF100101, 0xFF080101, 0xFF0A0102, 0xFF0C0202, 0xFF0D0203, 0xFF0F0203, 0xFF110203, 0xFF130204, 0xFF150204, 0xFF170204, 0xFF180305, 0xFF1A0305, 0xFF1C0305, 0xFF1E0306, 0xFF200406, 0xFF220407,
    ...
    // point size is strictly integer to enable building virtual render map for all rows/gaps
    ...
    private float TEXT_FONT_SIZE_DP = 12;
    ...
    // cps data
    private List<AtomSpectraSpectrogramData.SegmentData> segmentsData = null; // original data list
    private ArrayList<ArrayList<float[]>> segmentsBinData = null; // binned segments, cps data only
    ...
    private List<GapMeta> gapsMeta = null; // gaps information

    // pre-calculated virtual layout, as all segments be rendered into plain image without virtual scroll
    // answers if current virtual row belongs to segment or gap (and which exactly)
    private static final class VirtualRowMeta {
        final int segmentIndex; // references segmentsData[segmentIndex]
        final int segmentRowBinIndex; // references segmentsBinData[segmentIndex][segmentRowBinIndex]
        final int gapIndex; // references gapsMeta[gapIndex]

        private VirtualRowMeta(int segmentIndex, int segmentRowBinIndex) {
            this.segmentIndex = segmentIndex;
            this.segmentRowBinIndex = segmentRowBinIndex;
            this.gapIndex = -1;
        }

        private VirtualRowMeta(int gapIndex) {
            this.segmentIndex = -1;
            this.segmentRowBinIndex = -1;
    ...
    // region selection per segment
    public static final class SelectionBound {
        public final int segmentIndex;
        public final int rowIndex; // original index, not bin!

        private SelectionBound(int segmentIndex, int rowIndex) {
            this.segmentIndex = segmentIndex;
            this.rowIndex = rowIndex;
        }

        public static SelectionBound zeroIndex() {
            return new SelectionBound(0, 0);
        }

        // lexicographic order: segment first, then row within the segment
        public int compareTo(SelectionBound other) {
            if (segmentIndex != other.segmentIndex) {
                return Integer.compare(segmentIndex, other.segmentIndex);
            }
            return Integer.compare(rowIndex, other.rowIndex);
        }
    }

    private SelectionBound bgLeftHandleRow = null;
    ...
    private SelectionBound visibleEndRow = null;
    ...
    public SelectionBound getBgLeftHandleRow() {
        return bgLeftHandleRow;
    }

    public SelectionBound getBgRightHandleRow() {
        return bgRightHandleRow;
    }

    public SelectionBound getFgLeftHandleRow() {
        return fgLeftHandleRow;
    }

    public SelectionBound getFgRightHandleRow() {
        return fgRightHandleRow;
    }

    public int getVisibleStartChannel() {
        return visibleStartChannel;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (segmentsBinData == null || segmentsBinData.isEmpty()) {
    ...
    private SelectionBound getRowForSelectionHandle(int handle) {
        switch (handle) {
            case HANDLE_BG_LEFT:
                return bgLeftHandleRow;
    ...
    private void setSelectionHandleRow(int handle, SelectionBound row) {
        switch (handle) {
            case HANDLE_BG_LEFT:
                bgLeftHandleRow = row;
                break;
    ...
    private void ensureHandlesIncludeWholeBins() {
        // make sure range includes all rows from selected bin: earlier bound snaps to bin start, later bound to bin end
        if (bgLeftHandleRow.compareTo(bgRightHandleRow) <= 0) {
            bgLeftHandleRow = floorBoundToRowBinStart(bgLeftHandleRow);
            bgRightHandleRow = ceilBoundToRowBinEnd(bgRightHandleRow);
        } else {
            bgLeftHandleRow = ceilBoundToRowBinEnd(bgLeftHandleRow);
            bgRightHandleRow = floorBoundToRowBinStart(bgRightHandleRow);
        }

        if (fgLeftHandleRow.compareTo(fgRightHandleRow) <= 0) {
            fgLeftHandleRow = floorBoundToRowBinStart(fgLeftHandleRow);
            fgRightHandleRow = ceilBoundToRowBinEnd(fgRightHandleRow);
        } else {
            fgLeftHandleRow = ceilBoundToRowBinEnd(fgLeftHandleRow);
            fgRightHandleRow = floorBoundToRowBinStart(fgRightHandleRow);
        }
    }
    ...
    public void renderSpectrogram(AtomSpectraSpectrogramData data, int spectrumBinning, int channelBinning,
        ...
        this.segmentsBinData = new ArrayList<>(this.segmentsData.size());
        for (int s = 0; s < this.segmentsData.size(); s++) {
            AtomSpectraSpectrogramData.SegmentData segment = this.segmentsData.get(s);
            ArrayList<float[]> originalSpectrogram = segment.getSpectrogram();
            ...
            segmentsBinData.add(binnedSpectrogram);
        }
        ...
        for (int s = 0; s < this.segmentsData.size(); s++) {
            ...
            if (spectrumBinning > 1) {
                // insert binned segment to virtual rows
                ArrayList<Float> originalDurations = segment.getDurations();
                ArrayList<float[]> binnedSpectrogram = this.segmentsBinData.get(s);
                ...
                for (int i = 0; i < binnedSpectrogram.size(); i += spectrumBinning) {
                    ...
                    virtualRows.add(new VirtualRowMeta(s, spectrumBinnedSpectrogram.size() - 1));
                }

                this.segmentsBinData.set(s, spectrumBinnedSpectrogram);
            } else {
                // insert segment rows as is
                int segmentSize = this.segmentsBinData.get(s).size();
                for (int binIndex = 0; binIndex < segmentSize; binIndex++) {
                    virtualRows.add(new VirtualRowMeta(s, binIndex));
                }
            }

            previousSegment = segment;
        }
        this.virtualRowsMeta = virtualRows.toArray(new VirtualRowMeta[0]);

        this.spectrogramMaxCps = 0;
        for (ArrayList<float[]> segmentBinData : this.segmentsBinData) {
            for (float[] deltas : segmentBinData) {
    ...
    // "12m 34s", "45s"), collapsing to a single unit when the smaller one is zero.
    private String formatGapDuration(long millis) {
        ...
        if (days > 0) {
            return hours > 0
                    ? resources.getString(R.string.spectrogram_gap_duration_days_hours, days, hours)
                    : resources.getString(R.string.spectrogram_gap_duration_days, days);
        }
        if (hours > 0) {
            return minutes > 0
                    ? resources.getString(R.string.spectrogram_gap_duration_hours_minutes, hours, minutes)
                    : resources.getString(R.string.spectrogram_gap_duration_hours, hours);
        }
        if (minutes > 0) {
            return seconds > 0
                    ? resources.getString(R.string.spectrogram_gap_duration_minutes_seconds, minutes, seconds)
                    : resources.getString(R.string.spectrogram_gap_duration_minutes, minutes);
        }
        return resources.getString(R.string.spectrogram_gap_duration_seconds, seconds);
    }
    ...
    // IMPORTANT: binning arithmetic works only within a single segment
    ...
    // for a given bound calculates appropriate row-bin and returns row index for its end
    private SelectionBound ceilBoundToRowBinEnd(SelectionBound bound) {
        int bin = getBinForRow(bound.rowIndex, spectrumBinning);
        int row = getBinEndRow(bin, spectrumBinning);

        return new SelectionBound(bound.segmentIndex, row);
    }

    // for a given bound calculates appropriate row-bin and returns row index for its start
    private SelectionBound floorBoundToRowBinStart(SelectionBound bound) {
        int bin = getBinForRow(bound.rowIndex, spectrumBinning);
        int row = getBinStartRow(bin, spectrumBinning);
    ...
    private void renderSpectrogramToBitmap() {
        ...
        int maxVirtualRowsInView = spgViewHeight / virtualRowHeightPx;
        int maxColBinsInView = spgViewWidth / colBinWidthPx;

        // validate offset values
        int totalBinnedSpgHeight = virtualRowsCount * virtualRowHeightPx;
        if (totalBinnedSpgHeight < spgViewHeight) {
            ...
            int maxOffset = totalBinnedSpgHeight - maxVirtualRowsInView * virtualRowHeightPx;
        ...
        // render visible area
        int virtualRowStart = Math.max(0, verticalOffsetPx / virtualRowHeightPx);
        int virtualRowEnd = Math.min(virtualRowsCount - 1, virtualRowStart + maxVirtualRowsInView - 1);
        int virtualRowsToRender = virtualRowEnd - virtualRowStart + 1;
        int virtualRowsToRenderHeightPx = virtualRowsToRender * virtualRowHeightPx;
        ...
        for (int virtualRowIndex = virtualRowStart; virtualRowIndex <= virtualRowEnd; virtualRowIndex++) {
            ...
            if (rowMeta.segmentIndex >= 0) {
                int rowBin = rowMeta.segmentRowBinIndex;
                ArrayList<float[]> spectrogramBinData = this.segmentsBinData.get(rowMeta.segmentIndex);
                for (int colBin = colBinStart; colBin <= colBinEnd; colBin++) {
                    ...
                    int pxTopLeftIndex = (virtualRowIndex - virtualRowStart) * colBinsToRenderWidthPx * virtualRowHeightPx + (colBin - colBinStart) * colBinWidthPx;
                ...
                visibleGaps.add(rowMeta.gapIndex);
                for (int colBin = colBinStart; colBin <= colBinEnd; colBin++) {
                    int pxTopLeftIndex = (virtualRowIndex - virtualRowStart) * colBinsToRenderWidthPx * virtualRowHeightPx + (colBin - colBinStart) * colBinWidthPx;
                    for (int i = 0; i < POINT_SIZE_PX; i++) {
                        for (int j = 0; j < POINT_SIZE_PX; j++) {
                            spgPixels[pxTopLeftIndex + i * colBinsToRenderWidthPx + j] = GAP_BAND_COLOR;
                        }
                    }
                }
            }
        ...
        synchronized (spectrogramBitmapSync) {
            if (this.spectrogramBitmap != null) {
                this.spectrogramBitmap.setPixels(spgPixels, 0, colBinsToRenderWidthPx, TIME_AXIS_WIDTH_PX, PADDING_TOP_PX, colBinsToRenderWidthPx, virtualRowsToRenderHeightPx);
            }
        }

        // render gap labels
        renderGapLabels(visibleGaps, viewWidth, spgViewHeight);
        ...
        synchronized (this.spectrogramBitmapSync) {
            if (this.spectrogramBitmap != null) {
                ...
                for (int virtualRowIndex = virtualRowStart; virtualRowIndex <= virtualRowEnd; virtualRowIndex++) {
                    VirtualRowMeta rowMeta = virtualRowsMeta[virtualRowIndex];
                    if (rowMeta.gapIndex >= 0) {
                        // no timestamps for gaps
                        continue;
                    }
                    AtomSpectraSpectrogramData.SegmentData segment = this.segmentsData.get(rowMeta.segmentIndex);
                    ArrayList<Long> timestamps = segment.getTimestamps();
                    if (timestamps.isEmpty()) {
                        continue;
                    }

                    int tsBinIndex = rowMeta.segmentRowBinIndex;
                    int originalRowIndex = getBinEndRow(tsBinIndex, spectrumBinning);
                    if (originalRowIndex >= timestamps.size()) {
                        originalRowIndex = timestamps.size() - 1;
                    }

                    if (virtualRowIndex == virtualRowStart) {
                        // render date at the beginning of time axis
                        long rangeStartTimestamp = timestamps.get(originalRowIndex);
                    ...
                    long timestamp = timestamps.get(originalRowIndex);
    ...
    private void renderGapLabels(HashSet<Integer> visibleGaps, int viewWidth, int spgViewHeight) {
        if (visibleGaps.isEmpty() || this.gapsMeta == null) {
            return;
        }
        ...
        synchronized (this.spectrogramBitmapSync) {
            ...
            float centerX = TIME_AXIS_WIDTH_PX + (viewWidth - PADDING_RIGHT_PX - TIME_AXIS_WIDTH_PX) / 2f;
            float spgTop = PADDING_TOP_PX;
            float spgBottom = spgViewHeight + PADDING_TOP_PX;

            for (int gapIndex : visibleGaps) {
                GapMeta gap = this.gapsMeta.get(gapIndex);
                if (gap == null) {
                    continue;
                }
    ...
    // draws a single gap label line vertically centered on centerY, skipping it if it would cross the spectrogram area
    private void drawGapLabelLine(Canvas canvas, Paint paint, String text, float centerX, float centerY,
                                  Paint.FontMetrics fm, float spgTop, float spgBottom) {
        float baseline = centerY - (fm.ascent + fm.descent) / 2f;
        if (baseline + fm.ascent < spgTop || baseline + fm.descent > spgBottom) {
            // line would spill outside the spectrogram area (into padding/axes) - hide it
            return;
        }
        canvas.drawText(text, centerX, baseline, paint);
    }

    private void setVisibleRangeSnapshot(int virtualRowStart, int virtualRowEnd, int colBinStart, int colBinEnd) {
        // snapshot for hit-testing and listener
        VirtualRowMeta rowStartMeta = virtualRowsMeta[virtualRowStart];
        if (rowStartMeta.gapIndex >= 0) {
            // move forward until segment or boundary found
            while (++virtualRowStart <= virtualRowEnd && virtualRowStart < virtualRowsMeta.length) {
                rowStartMeta = virtualRowsMeta[virtualRowStart];
                if (rowStartMeta.segmentIndex >= 0) {
                    break;
                }
            }
        }
        if (rowStartMeta.segmentIndex >= 0) {
            this.visibleStartRow = new SelectionBound(rowStartMeta.segmentIndex, getBinStartRow(rowStartMeta.segmentRowBinIndex, spectrumBinning));
        } else {
            this.visibleStartRow = null;
        }

        VirtualRowMeta rowEndMeta = virtualRowsMeta[virtualRowEnd];
        if (rowEndMeta.gapIndex >= 0) {
            // move backward until segment or boundary found
            while (--virtualRowEnd >= virtualRowStart && virtualRowEnd > 0) {
                rowEndMeta = virtualRowsMeta[virtualRowEnd];
                if (rowEndMeta.segmentIndex >= 0) {
                    break;
                }
            }
        }
        if (rowEndMeta.segmentIndex >= 0) {
            this.visibleEndRow = new SelectionBound(rowEndMeta.segmentIndex, getBinEndRow(rowEndMeta.segmentRowBinIndex, spectrumBinning));
        } else {
    ...
    private void drawHandle(Canvas canvas, Paint guidePaint, Paint trianglePaint,
                            int viewWidth, SelectionBound row, boolean leftSide, int color) {
        if (row == null || visibleStartRow == null || visibleEndRow == null) {
            return;
        }
